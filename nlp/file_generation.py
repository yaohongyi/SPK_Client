...
class FileGeneration(QtCore.QThread):
    ...
    def create_documents_info(self):
        """生成证件信息"""
        documents_info = dict()
        if self.client_info.get('id_card'):
            documents_info['id_card'] = f'身份证：{nlp_tool.create_id_card()}'  # 国内身份证
        if self.client_info.get('hk_mc_pass'):
            documents_info['hk_mc_pass'] = f'港澳通行证：{nlp_tool.create_land_to_hm_pass()}'  # 港澳通行证
        if self.client_info.get('tw_pass'):
            documents_info['tw_pass'] = f'台湾通行证：{nlp_tool.create_land_to_tw_pass()}'  # 台湾通行证
        if self.client_info.get('passport'):
            documents_info['passport'] = f'护照号：{nlp_tool.create_passport()}'  # 护照
        if self.client_info.get('hk_id_card'):
            documents_info['hk_id_card'] = f'香港身份证：{nlp_tool.create_hk_id()}'  # 香港身份证
        if self.client_info.get('hk_homecoming_permit'):
            documents_info['hk_homecoming_permit'] = f'香港回乡证：{nlp_tool.create_hk_to_land_pass()}'  # 香港回乡证
        if self.client_info.get('mc_id_card'):
            documents_info['mc_id_card'] = f'澳门身份证：{nlp_tool.create_mc_id()}'  # 澳门身份证
        if self.client_info.get('mc_homecoming_permit'):
            documents_info['mc_homecoming_permit'] = f'澳门回乡证：{nlp_tool.create_mc_to_land_pass()}'  # 澳门回乡证
        if self.client_info.get('tw_id_card'):
            documents_info['tw_id_card'] = f'台湾身份证：{nlp_tool.create_tw_id()}'  # 台湾身份证
        if self.client_info.get('tw_homecoming_permit'):
            documents_info['tw_homecoming_permit'] = f'台湾回乡证：{nlp_tool.create_tw_to_land_pass()}'  # 台湾回乡证
        if self.client_info.get('foreign_passport'):
            documents_info['foreign_passport'] = f'外国人护照：{nlp_tool.create_passport()}'  # 外国人护照
    ...
    def create_content(self):
        ...
        if people_relation == '无':
            ...
            for x in range(people_num):
                people_info = self.create_all_info()
                attribute_list = []
            ...
            people_info_1 = self.create_all_info()
            people_info_2 = self.create_all_info()
    ...
    def create_content_fixed_documents(self, documents):
        """创建，证件是固定的文本内容"""
        brigade = nlp_tool.create_brigade_info()  # 大队信息
        ...
        for _, attribute in people_info.items():
            attribute_list.append(attribute)
        documents_list = list()
        for _, documents_value in documents.items():
            documents_list.append(documents_value)
        attribute_list[1:1] = documents_list
        single_people_desc = '，'.join(attribute_list)
        case_desc = f"2020年12月25日，在广东省深圳市开展的一次特别活动，代号“秃鹰”，旨在打击法轮功等邪教组织。" \
            f"在领导的有效指挥，和各级部门的配合下，打击效果明晓，获得社会大众认可！其中，抓到了多名邪教骨干。" \
            f"现将涉案嫌疑人信息公布如下：\n{single_people_desc}。\n" \
    ...
    def file_generation(self):
        """"""
        file_type = self.client_info.get('file_type')
        file_num = self.client_info.get('file_num')
        dir_path = self.client_info.get('save_path')
        fixed_documents = self.client_info.get('fixed_documents')
        now_time = str(int(time.time() * 1000000))
        self.text.emit(f'即将在{dir_path}目录生成文件：\n')
        if fixed_documents == '不固定':
            for x in range(1, file_num + 1):
                case_desc = self.create_content()
                if file_type == 'txt':
                    with open(f'{dir_path}/txt_{now_time}_{x}.txt', 'w', encoding='utf-8') as f:
                        f.write(case_desc)
                else:
                    doc_document = docx.Document()
                    doc_document.add_heading('关于2019毒品目标案件的梳理报道', 0)
                    doc_document.add_paragraph(case_desc)
                    doc_document.save(f'{dir_path}/docx_{now_time}_{x}.docx')
                self.text.emit(f'成功生成第{x}个文件！\n')
        # 多个文档中的同一个人员固定证件信息
        elif fixed_documents == '固定':
            documents = self.create_documents_info()
            for x in range(1, file_num + 1):
                case_desc = self.create_content_fixed_documents(documents)
                if file_type == 'txt':
                    with open(f'{dir_path}/txt_{now_time}_{x}.txt', 'w', encoding='utf-8') as f:
                        f.write(case_desc)
                else:
